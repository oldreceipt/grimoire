...
export async function loadGltfPreview(url: string): Promise<GLTF> {
  ...
  try {
    ...
    // Resolve the NPR mask texture indices (the only part of the morphic contract
    // the stock loader does not surface) while gltf.parser is still live and the
    // ImageBitmap suppression is in effect. No-op when no material carries masks.
